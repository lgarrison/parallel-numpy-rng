...
def generate_int_to_float(bitgen):
    ...
    def _next_float_pcg64(state):
        ...
        return np.float32(
            np.int32(_next_uint32_pcg64(state) >> np.uint32(8))
            * (np.float32(1.0) / np.float32(16777216.0))
        )
    ...
    def _next_float_pcg64_nonzero(state):
        """Random float in the semi-open interval (0,1]"""
        return np.float32(
            (np.int32(_next_uint32_pcg64(state) >> np.uint32(8)) + np.int32(1))
            * (np.float32(1.0) / np.float32(16777216.0))
        )
    ...
    def _next_double_pcg64(state):
        ...
        return np.float64(
            np.int64(_next_uint64_pcg64(state) >> np.uint64(11))
            * (np.float64(1.0) / np.float64(9007199254740992.0))
        )
    ...
    def _next_double_pcg64_nonzero(state):
        """Random double in the semi-open interval (0,1]"""
        return np.float64(
            (np.int64(_next_uint64_pcg64(state) >> np.uint64(11)) + np.int64(1))
            * (np.float64(1.0) / np.float64(9007199254740992.0))
        )

    _next_float = {
        "zero": {np.float32: _next_float_pcg64, np.float64: _next_double_pcg64},
        "nonzero": {
            np.float32: _next_float_pcg64_nonzero,
            np.float64: _next_double_pcg64_nonzero,
        },
    }
    return _next_float
