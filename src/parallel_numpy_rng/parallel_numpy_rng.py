...
class MTGenerator:
    ...
    def __init__(self, bit_generator, nthread=-1):
        ...
        self._next_float_nonzero = _next_float[bit_generator.state["bit_generator"]][
            "nonzero"
        ]
    ...
    def random(
        self, size=None, nthread=None, out=None, verify_rng=True, dtype=np.float64
    ):
        if size is None:
            size = 1
        if nthread is None:
    ...
    def standard_normal(
        ...
        ff = 2 * np.linspace(
            0, (size + 1 - first) // 2, num=nthread + 1, endpoint=True, dtype=np.int64
        )
